...
const MorphingTextShader = {
  ...
  vertexShader: `
    ...
    void main() {
      ...
      // Localize the effect: we want it to react mostly near the mouse cursor
      float localMask = smoothstep(0.4, 0.0, distToMouse); 
      
      // Combine masks so text only bends where the wave exists AND we stroked
      float mask = waterMask * localMask;

      // Intense shear based on mouse movement velocity
      // If we move the mouse right (uMouseVel.x is positive), shearX is positive
      float shearX = uMouseVel.x * 250.0;
      float shearY = uMouseVel.y * 150.0;
...
const WaterVisualShader = {
  ...
  fragmentShader: `
    ...
    void main() {
      ...
      // Stronger Chromatic Aberration for that "liquid" look
    ...
    <>
      ...
      {createPortal(
        <Text 
          ...
          fontSize={Math.min(viewport.width * 0.22, 0.65)} 
